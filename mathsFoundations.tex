...
\hline
\texttt{evaluate(o)} & Calculate the expectation value of obervation operator o\\ 
\hline
\end{tabular}
...
An \texttt{ABM} object represents a probability distribution over the states of an agent based model. To be mathematically precise, an \texttt{ABM} belongs to a symmetric Fock space on the state space of the agents, $T$, which we'll write as $\mathcal{F}(T)$. However, it is not at all obvious how we could go about representing such a mathematical object as a data structure for numerical computation.
...
We can now interpret the \texttt{add(s)} operation as the creation operator $a^\dag_s$.
...
\]
and we see that the \texttt{Behaviour} takes the place of the Hamiltonian when seen as a dynamical system. This is significant because the distribution, $P$, will more often than not become very difficult to represent as a data structure, but the Hamiltonian is very easy to represent and construct from the model code. It also means that we can express the meaning of the \texttt{integrate} method as
\begin{equation}
...
We can now show how our operators extend very elegantly to systems with more than one agent and add state to the agents. A probability distribution for a set of agents can be represented as a set of operators on the empty set, we are left with something like a polynomial:
\[
...
\label{timeint}
\end{equation}
can be given an alternative interpretation, as a path integral. In order to see this, we need to introduce the dot product of two ABM probability distributions, which we define as
\[
