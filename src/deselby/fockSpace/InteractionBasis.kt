    ...
    override fun commute(basis: CreationBasis<AGENT>, termConsumer: (Basis<AGENT>, Double) -> Unit) {
        ...
        if(m1 != null) {
            if(m2 != null) {
                ...
                val creationsminusd1 = creationUnion.plus(d1,-1)
                termConsumer(ActionBasis(creationsminusd1, d2), m1.toDouble())
                termConsumer(ActionBasis(creationUnion.plus(d2,-1), d1), m2.toDouble())
                termConsumer(CreationBasis(creationsminusd1.plus(d2,-1)), (m1*m2).toDouble())
            } else {
                termConsumer(ActionBasis(this.creations union basis.creations.plus(d1,-1), d2), m1.toDouble())
            }
        } else if(m2 != null) {
            termConsumer(ActionBasis(this.creations union basis.creations.plus(d2,-1), d1), m2.toDouble())
        }
    }
